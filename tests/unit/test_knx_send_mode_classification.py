...
class TestIntervalsFromTimestamps:
    ...
    def test_two_timestamps_one_interval(self) -> None:
        result = intervals_from_timestamps(["2026-05-03T08:00:00", "2026-05-03T08:00:30"])
        assert result == [30.0]

    def test_unsorted_input_is_sorted(self) -> None:
        result = intervals_from_timestamps(["2026-05-03T08:00:30", "2026-05-03T08:00:00"])
        assert result == [30.0]

    def test_duplicate_timestamps_yield_zero_interval(self) -> None:
        result = intervals_from_timestamps(["2026-05-03T08:00:00", "2026-05-03T08:00:00"])
        assert result == [0.0]
...
class TestClassifySendMode:
    ...
    def test_few_telegrams_returns_insufficient(self) -> None:
        result = classify_send_mode([60.0] * 4, sample_count=5, value_changes=0)
    ...
    def test_regular_intervals_classify_as_cyclic(self) -> None:
        ...
        result = classify_send_mode(intervals, sample_count=36, value_changes=0)
    ...
    def test_irregular_with_value_changes_returns_on_change(self) -> None:
        ...
        result = classify_send_mode(intervals, sample_count=27, value_changes=25)
    ...
    def test_hybrid_when_no_clear_pattern(self) -> None:
        ...
        result = classify_send_mode(intervals, sample_count=33, value_changes=5)
    ...
    def test_medium_confidence_just_below_high_threshold(self) -> None:
        intervals = [60.0] * 20
        result = classify_send_mode(intervals, sample_count=21, value_changes=0)
    ...
    def test_zero_median_falls_back_to_hybrid(self) -> None:
        ...
        intervals = [0.0] * 20
        result = classify_send_mode(intervals, sample_count=21, value_changes=10)
        assert result.mode == "hybrid"
...
async def _insert_raw(db: Database, *, ga: str, ts: str, value: object = 1) -> None:
    await db.execute(
...
class TestSamplesForGaClassification:
    ...
    async def test_empty_ga_returns_empty(self, db: Database) -> None:
        ...
        result = await repo.samples_for_ga_classification("", _ts(-3600), _ts(0))
        assert result == []

    @pytest.mark.asyncio
    async def test_no_telegrams_returns_empty(self, db: Database) -> None:
        repo = KnxStatsRepository(db)
        result = await repo.samples_for_ga_classification("1/2/3", _ts(-3600), _ts(0))
        assert result == []
    ...
    async def test_returns_chronological_order(self, db: Database) -> None:
        ...
        repo = KnxStatsRepository(db)
        result = await repo.samples_for_ga_classification("1/2/3", _ts(-1), _ts(100))
        timestamps = [r["timestamp"] for r in result]
        assert timestamps == sorted(timestamps)

    @pytest.mark.asyncio
    async def test_only_returns_telegrams_in_period(self, db: Database) -> None:
        ...
        repo = KnxStatsRepository(db)
        result = await repo.samples_for_ga_classification("1/2/3", _ts(-1), _ts(120))
        assert len(result) == 2
    ...
    async def test_does_not_return_other_gas(self, db: Database) -> None:
        ...
        repo = KnxStatsRepository(db)
        result = await repo.samples_for_ga_classification("1/2/3", _ts(-1), _ts(60))
        assert len(result) == 1
    ...
    async def test_hard_cap_respected(self, db: Database) -> None:
        ...
        repo = KnxStatsRepository(db)
        result = await repo.samples_for_ga_classification("1/2/3", _ts(-1), _ts(300), limit=50)
        assert len(result) == 50

    @pytest.mark.asyncio
    async def test_hard_cap_clamped_to_safety_max(self, db: Database) -> None:
