...
async def test_messages_1h_window_excludes_older(repo: MessageRepository) -> None:
    ...
    now = datetime.now(UTC)
    await repo.insert(Message(severity=Severity.INFO, source="x", text="alt",
                              timestamp=now - timedelta(hours=2)))
    await repo.insert(Message(severity=Severity.INFO, source="x", text="frisch",
                              timestamp=now - timedelta(minutes=10)))
    await repo.insert(Message(severity=Severity.ERROR, source="x", text="frisch2",
...
async def test_messages_7d_window_includes_today(repo: MessageRepository) -> None:
    ...
    now = datetime.now(UTC)
    await repo.insert(Message(severity=Severity.INFO, source="x", text="vor 8 Tagen",
                              timestamp=now - timedelta(days=8)))
    for d in (6, 3, 1, 0):
        await repo.insert(Message(severity=Severity.INFO, source="x", text=f"vor {d}d",
